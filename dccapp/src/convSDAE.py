...
class convSDAEcic(nn.Module):
    def __init__(self, dim, output_padding, numpen, dropout=0.2, slope=0.0):
        super(convSDAEcic, self).__init__()
        self.in_dim = dim[0]
        self.nlayers = len(dim)-1
        self.reluslope = slope
        self.numpen = numpen
        ...
        for i in range(self.nlayers):
            if i == self.nlayers - 1:  # 3
                ...
                self.enc.append(nn.Conv2d(dim[i], dim[i + 1], kernel_size=(33,1), stride=2, padding=(16, 0)))
                self.benc.append(nn.BatchNorm2d(dim[i + 1]))
                self.dec.append(nn.ConvTranspose2d(dim[i+1], dim[i], kernel_size=(33,1), stride=2, padding=(16, 0),
                                                   output_padding=output_padding[i]))
                self.bdec.append(nn.BatchNorm2d(dim[i]))
            else:
                self.enc.append(nn.Conv2d(dim[i], dim[i + 1], kernel_size=(33,1), stride=2, padding=(16, 0)))
