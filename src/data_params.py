...
def extract_sdae_smmc(slope=0.0, dim=10):
    return extractSDAE(dim=[960, 500, 500, 2000, dim], slope=slope)

def extract_sdae_cic(slope=0.0, dim=10):
    return extractSDAE(dim=[768, 500, 500, 2000, dim], slope=slope)
...
def sdae_smmc(dropout=0.2, slope=0.0, dim=10):
    return SDAE(dim=[960, 500, 500, 2000, dim], dropout=dropout, slope=slope)

def sdae_cic(dropout=0.2, slope=0.0, dim=10):
    return SDAE(dim=[768, 500, 500, 2000, dim], dropout=dropout, slope=slope)
...
def load_predefined_net(args, params):
    if args.db == 'mnist':
        ...
        net = sdae_smmc(dropout=params['dropout'], slope=params['reluslope'], dim=args.dim)
    elif args.db == 'cic':
        net = sdae_cic(dropout=params['dropout'], slope=params['reluslope'], dim=args.dim)
...
def load_predefined_extract_net(args):
    ...
    if args.db == 'mnist':
        ...
        net = extract_sdae_smmc(slope=reluslope, dim=args.dim)
    elif args.db == 'cic':
        net = extract_sdae_cic(slope=reluslope, dim=args.dim)
    elif args.db == 'reuters' or args.db == 'rcv1':
