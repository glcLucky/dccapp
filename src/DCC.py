...
parser.add_argument('--filename', type=str, default='traindata', help='the file name of data')
parser.add_argument('--version', '-v', type=str, default='default', help='the version of pretrain config')
parser.add_argument('--pretrain_fname', type=str, default='pretrained', help='the file name of pretrained file')
...
def main(args, net=None):
    ...
    outputdir = get_output_dir(args.db, version=args.version)
    ...
    trainset = DCCPT_data(root=datadir, filename=args.filename, h5=args.h5)
    testset = DCCPT_data(root=datadir, filename=args.filename, h5=args.h5)
    numeval = len(trainset) + len(testset)

    # extracting training data from the pretrained.mat file
    data, pairs, Z, sampweight = makeDCCinp(args)
    ...
    output = {'Z': Z, 'U': U, 'w': pairs, 'cluster':assignment}
    sio.savemat(os.path.join(datadir, args.version, 'features'), output)
...
def test(testloader, net, criterion, epoch, use_cuda, _delta, pairs, numeval, flag, logger, viz_method):
    ...
    original, features = np.asarray(original).astype(np.float32), np.asarray(features).astype(np.float32)

    U = criterion.U.data.cpu().numpy()

    assignment = np.zeros(len(U))
    change_in_assign = 0
    ...
    if flag:
        index, n_components, assignment = computeObj(U, pairs, _delta, numeval)
