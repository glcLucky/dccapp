...
def main(args, net=None):
    ...
    dataloader = torch.utils.data.DataLoader(trainset, batch_size=100, shuffle=False, **kwargs)
    ...
    features, features_dr,  = extract(dataloader, net, use_cuda)
    print('Done.\n')

    feat_path = os.path.join(datadir, args.version, args.feat)
    if args.h5:
        import h5py
        ...
        pickle.dump({'Z': np.squeeze(features_dr), 'data': np.squeeze(features)}, fo, protocol=2)
        fo.close()
    return features, features_dr
...
def extract(dataloader, net, use_cuda):
    ...
    return original, np.asarray(features).astype(np.float32)
