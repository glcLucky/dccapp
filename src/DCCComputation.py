...
def makeDCCinp(args):
    ...
    datafile = args.pretrain_fname

    if args.h5:
        datafile = os.path.join(datadir, args.version, datafile+'.h5')
    else:
        datafile = os.path.join(datadir, args.version, datafile+'.mat')
...
def computeHyperParams(pairs, Z):
    ...
    _delta2 = float(np.average(epsilon[:robsamp]) / 2)  # the mean of the bottom 1% of the pairwise distances in epsilon 
...
def computeObj(U, pairs, _delta, numeval):
    ...
    return index2, n_components, labels
